...
public class DecalSystem : MonoBehaviour
{
    ...
    public DecalConfigSO decalsConfig;
    ...
    private void DrawDecalProjector(Vector3 position, Quaternion rotation, RaycastHit hit, DecalsOudated _decals)
    {
        //GameObject projector = ObjectPooler.instance.GetPooledObject();
        GameObject projector = ObjectPoolManager.SpawnObject(decalProjector, hit.collider.transform, position, 
            Quaternion.FromToRotation(Vector3.forward, -hit.normal), ObjectPoolManager.PoolType.Decals);
        ObjectPoolManager.SpawnObject(_decals.particle, hit.collider.transform, position, rotation, ObjectPoolManager.PoolType.ParticleSystems);
        if (projector == null) return;
        if (_decals.decals != null)
        {
            ...
            int randIndexMat = Random.Range(0, _decals.decals.Length);
            Material randMaterial = _decals.decals[randIndexMat];

            DecalProjector _projector = projector.GetComponentInChildren<DecalProjector>();
            _projector.material = randMaterial;
            _projector.size = new Vector3(_decals.decalSize, _decals.decalSize, _decals.decalDepth);
            _projector.pivot = new Vector3(0, 0, _decals.decalDepth / 2f);

            //SetParentObject(projector, hit.collider.gameObject);
        }
        if (_decals.playSound != null)
        {
            AudioSource _sound = projector.GetComponentInChildren<AudioSource>();
            _sound.clip = _decals.playSound[Random.Range(0, _decals.playSound.Length)];
            _sound.Play();
        }
    }
    ...
    private void DrawSplatterProjector(Vector3 position, Quaternion rotation, GameObject splatterParent, RaycastHit hit, SplatterDecals _splatterDecals)
    {
        ...
        {
            //GameObject projector = ObjectPooler.instance.GetPooledObject();
            GameObject projector = ObjectPoolManager.SpawnObject(decalProjector, position, Quaternion.FromToRotation(Vector3.forward, -hit.normal), ObjectPoolManager.PoolType.Decals);
            if (projector != null)
            {
                /*projector.transform.position = position;
                projector.transform.rotation = Quaternion.FromToRotation(Vector3.forward, -hit.normal);
    ...
    /// <returns></returns>
    private DecalsOudated CheckOverrides(RaycastHit hit)
    {
        DecalsOudated defaultDecals = null;

        for (int i = 0; i < decalsConfig.decals.Length; i++)
        {
            if (decalsConfig.decals[i].overrideTag == "")
            {
                defaultDecals = decalsConfig.decals[i];
            }

            if(hit.collider.CompareTag(decalsConfig.decals[i].overrideTag))
            {
                return decalsConfig.decals[i];
            }
        }

        if (decalsConfig.decals.Length > 0)
        {
            defaultDecals = decalsConfig.decals[0];
        }
    ...
    private SplatterDecals CheckSplatterOverrides(RaycastHit hit)
    {
        ...
        for (int i = 0; i < splatterDecals.Length; i++)
        {
            ...
            if (hit.collider.CompareTag(splatterDecals[i].overrideTag))
            {
    ...
    private void SpawnBleedParticle(Vector3 position, Quaternion direction, GameObject parent)
    {
        ...
        if (randomNumber <= bleedChance)
        {
            ...
            //ObjectPoolManager.SpawnObject(bleedParticleSystem, bleedParent.transform, position, direction, ObjectPoolManager.PoolType.ParticleSystems);
    ...
    private void SpawnSplatterParticle(Vector3 postion, Vector3 direction, GameObject parent)
    {
        ...
        //ObjectPoolManager.SpawnObject(splatterParticleSystem, splatterParent.transform, postion, Quaternion.FromToRotation(Vector3.forward, direction), ObjectPoolManager.PoolType.ParticleSystems);
